...
application.config['AWS_PATH'] = 'item-catalog-uploads/'
application.config['AWS_HOST'] = 'https://s3-us-west-2.amazonaws.com/elasticbeanstalk-us-west-2-369336360970/item-catalog-uploads/'
...
def preventOverwrite(filename):
    ...
    i = 1
    f = filename.rsplit('.', 1)
    while os.path.isfile(os.path.join(application.config['UPLOAD_FOLDER'], filename)):
...
def googleConnect(token):
    ...
    CLIENT_ID = json.loads(
        open(application.root_path + '/client_secrets/google.json', 'r').read())['web']['client_id']

    try:
        # Upgrade the authorization token into a credentials object
        oauth_flow = flow_from_clientsecrets(application.root_path + '/client_secrets/google.json',
                                             scope='')
    ...
    if result['issued_to'] != CLIENT_ID:
        return {"response": "Token's client ID does not match application's.",
                "status": 401}
...
def facebookConnect(token):
    ...
    """
    h = httplib2.Http()
    application_id = json.loads(open(application.root_path + '/client_secrets/facebook.json', 'r').read())[
        'web']['application_id']
    application_secret = json.loads(
        open(application.root_path + '/client_secrets/facebook.json', 'r').read())['web']['application_secret']
...
def githubConnect(token):
    ...
    """
    h = httplib2.Http()
    client_id = json.loads(open(application.root_path + '/client_secrets/github.json', 'r')
                           .read())['client_id']
...
def getURLImage(filename):
    return application.config['AWS_HOST'] + filename
...
def uploaded_file(filename):
    ...
    #     filename,
    #     os.path.join(application.config['AWS_PATH'], filename))
...
def showLogin():
    ...
    client_id = {
        ...
        'facebook': loadClientSecret('facebook')['web']['application_id'],
...
@application.route('/acctconnect', methods=['POST'])
def acctConnect():
    """
...
@application.route('/acctdisconnect')
def acctDisconnect():
    """
    ...
    if(endpoint == 'json'):
        ...
        for val in items:
            if 'items' in categories[val.category_id]:
                categories[val.category_id]['items'].applicationend(serializeItem(val))
            else:
...
@application.route('/catalog/<int:cat_id>/')
@application.route('/catalog/<int:cat_id>/<endpoint>')
...
@application.route('/item/<int:item_id>/<endpoint>')
...
@application.route('/item/edit/<int:item_id>/',
           methods=['GET', 'POST'])
            ...
            if file and (file.filename != '') and allowedFile(file.filename):
                ...
                if item.image is not None:
                    os.remove(os.path.join(
                        application.config['UPLOAD_FOLDER'], item.image))

                imagefile = secure_filename(file.filename)
                imagefile = preventOverwrite(imagefile)
                file.save(os.path.join(application.config['UPLOAD_FOLDER'], imagefile))
...
@application.route('/item/add', methods=['GET', 'POST'])
def addItem():
    """
    Page to add an item
    """
    ...
    if request.method == 'POST':
        ...
        if 'image' in request.files:
            ...
            if file and (file.filename != '') and allowedFile(file.filename):
                ...
                imagefile = preventOverwrite(imagefile)
                # file.save(os.path.join(application.config['UPLOAD_FOLDER'], imagefile))
                # print(os.path.join(application.config['AWS_PATH'], imagefile))
                s3.meta.client.upload_fileobj(file, application.config['AWS_BUCKET'],os.path.join(application.config['AWS_PATH'], imagefile))
                # s3.meta.client.upload_file(
                #     imagefile,
                #     application.config['AWS_BUCKET'],imagefile)
                #print(type(file.read()))

                # s3.meta.client.upload_file(os.path.join(application.config['AWS_PATH'], imagefile), application.config['AWS_BUCKET'],imagefile)
...
@application.route('/item/delete/<int:item_id>/',
           methods=['POST'])
...
def addAWSHost():
    return dict(aws_host=application.config['AWS_HOST'])
